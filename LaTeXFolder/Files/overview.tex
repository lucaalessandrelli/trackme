...
\begin{enumerate}
\item Third party: Company interested in retrieve useful data from TrackMe's users. Usually, this information can be relevant for marketing strategy.

\item User: Individual whose data are acquired from TrackMe through Data4Help serivce and are provided to third parties. AutomatedSOS is a service thought for elderly people, while Track4Run is a service thought for atheltes, promoter and spectator of runs. User's privacy is protected by each service.

\end{enumerate}

\subsection{Assumptions, dependencies and constraints}
In the specification document certain parts are not specificied and a bit ambiguous. So we decided to make the following assumptions.
...
\item[•] {\Large AutomatedSOS}
    ...
    \item User can see all personal informations that have been sent to the Data4Help service. 
	\end{enumerate}
...
\item[•] {\Large Data4Help}
	\begin{enumerate}
	\item [D.1] User's informations are collected from partner applications or from the other two TrackMe applications installed on users' devices.
