...
\item[•]{\Large Data4Help}
	...
	\item [G.3] \textbf{Allow third parties two different ways to get users' data.}
    	...
    	\item [R.16] Collect all the useful data retrieved by Data4Help that are produced by the interested users. 
    	...
    	\item [R.16] Collect all the useful data retrieved by Data4Help that are produced by the interested users.
    	\item [R.19] Send all the collected information to request applicant.
    	\end{enumerate}
    	\end{enumerate}
    	
    \item [G.4] \textbf{Provide data in an anonymous way, to protect users' privacy.}
		\begin{enumerate}
    	\item [R.17] Deny third parties to receive single mode information about users that have not accepted the second part of the privacy policy.
    	\item [R.21] Deny third parties to receive information if the provided information can hurt users' privacy, for this purpose group request under 1000 users involved are rejected.
    	\end{enumerate}	
...
\item[•]{\Large AutomatedSOS}
	...
	\item [G.5] \textbf{Retrieve user's position and health status.}
		\begin{enumerate}
		\item [R.22] Allow users to be tracked from AutomatedSOS filling up the registration and agreeing only to first part of privacy policy.
		...
		\item [R.23] The application has to interact with Smartwatch/Smartphone APIs in order to retrieve location and health status.
		\end{enumerate}
	
	
	\item [G.6] \textbf{Monitor user's health parameters.}
		\begin{enumerate}
		\item [R.24] The application has to interact with Smartwatch APIs in order to retrieve location and health status.
		\item [R.25] The application has to retrieve users' health status every 2 seconds in order to guarantee a reaction time of 5 seconds.
		\item [R.26] The application sends to Data4Help service all the data retrieved in live acquisition.
		\item [R.27] The application gets from Data4Help service all the historical data about the user.
		\item [R.28] Allow the user to set personal threshold values.
		\end{enumerate}
		
	\item [G.7] \textbf{Send an ambulance to user's location whenever certain parameters are below the threshold.}
		\begin{enumerate}
		\item [R.29] The application has to control health status with data retrieved in local to immediately realize whether certain parameters are critical.
		\item [R.30] The application sends an ambulance request to the nearest hospital whenever parameters are critical.
		\item [D.10] The first aid system is always up and ready to receive messages from AutomatedSOS.
		\item [R.31] Supply to the hospital the user's location and all the useful information to provide efficient first aid.
		\item [R.32] In the case no answer arrives from the hospital the software must repeat another time the request until an answer in reached.
		\item [R.33] As soon as the acknowledgement message is received a warning message is displayed on the user's smartwatch.
...
\item[•]{\Large Track4Run}
	...
	\item [G.5] \textbf{Retrieve user's position and health status.}
		\begin{enumerate}
		\item [R.34] Allow users to be tracked from Track4Run filling up the registration and agreeing to privacy policy (both can be accepted).
		\item [D.4] Devices used to monitor individuals always report correct values.
