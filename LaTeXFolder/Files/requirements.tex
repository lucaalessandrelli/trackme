...
\item[•]{\Large AutomatedSOS}
	...
	\item [G.6] \textbf{Monitor user's health parameters.}
		...
		\item [R.26] Allow the user to set personal threshold values.
		\end{enumerate}
		
	\item [G.7] \textbf{Send an ambulance to user's location whenever certain parameters are below the threshold.}
		\begin{enumerate}
		\item [R.27] The application has to control health status with data retrieved in local to immediately realize whether certain parameters are critical.
		\item [R.28] The application sends an ambulance request to the nearest hospital whenever parameters are critical.
		\item [D.10] The first aid system is always up and ready to receive messages from AutomatedSOS.
		\item [R.29] Supply to the hospital the user's location and all the useful information to provide efficient first aid.
		\item [R.30] In the case no answer arrives from the hospital the software must repeat another time the request until an answer in reached.
		\item [R.31] As soon as the acknowledgement message is received a warning message is displayed on the user's smartwatch.
...
\item[•]{\Large Track4Run}
	...
	\item [G.5] \textbf{Retrieve user's position and health status.}
		\begin{enumerate}
		\item [R.32] Allow users to be tracked from Track4Run filling up the registration and agreeing to privacy policy (both can be accepted).
		\item [D.4] Devices used to monitor individuals always report correct values.
		\item [R.22] The application has to interact with Smartwatch/Smartphone APIs in order to retrieve location and health status.
		\end{enumerate}
	...
	\item [G.8] \textbf{Allow user to manage a run.}
		\begin{enumerate}
		\item [R.33] Allow users to create a run providing all the general information about the competition.
		\item [D.4] Devices used to monitor individuals always report correct values.
			
		\item [G.8.1] \textbf{Allow promoters to define a path for the run.}
			\begin{enumerate}
			\item [R.34] Allow promoters to define a path for the run by selecting the routes inside a map.
			\item [D.14] The path defined by the organizer actually exist.
			\end{enumerate}
			
		\item [G.8.2] \textbf{Allow promoters to invite athletes to the run.}
			\begin{enumerate}
			\item [R.35] Allow promoters to send a participation request.
			\item [R.36] Allow promoters to specify maximum number of athletes that can participate.
			\end{enumerate}
	\end{enumerate}
	
	\item [G.9] \textbf{Allow athlete to enroll on a specific run.}
		\begin{enumerate}
		\item [R.37] Allow the user to see all the runs generated (which he is invited or not).
		\item [R.38] Allow user to enroll in a run.
		\item [R.39] Deny user to enroll in a run if maximum number of participants is already reached.
	...
	\item [G.10] \textbf{Allow spectators to watch in real time the position of every athletes in a specific run.}
		...
		\item [R.40] Allow user to select a run to be viewed.
		\item [R.41] The application requests to Data4Help the position of all the other athletes involved.
		\item [R.42] The application receives and displays the position of all the other athletes involved.
