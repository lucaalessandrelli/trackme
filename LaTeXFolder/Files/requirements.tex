...
On the Website, thought for the third parties, it is possible both to made group and individual request and to view all the data provided to the specific third party. The third parties in order to send a data request must register through the registration function offered by the Website.
\bigbreak
\noindent
...
\item[•]{\Large Data4Help}
	...
	\item [G.1] \textbf{Acquire user's position and health status.}
		\begin{enumerate}
		\item [D.1] User's data are collected from partner applications or from the other two TrackMe applications installed on users' devices.
    ...
    \item [G.2] \textbf{Provide to third parties, the user's position and health status.}
		...
		\item [R.9] The system has to collect inside its database all the useful information that match the request.
	...
	\item [G.3] \textbf{Allow third parties two different ways to get users' data.}
		...
		\begin{enumerate}
    	\item [R.18] Allow third parties to insert search area and attributes in which they are interested to restrict their field of search.
...
\item[•]{\Large AutomatedSOS}
	...
	\item [G.5] \textbf{Retrieve user's position and health status.}
		\begin{enumerate}
		\item [R.20] Allow users to be tracked from AutomatedSOS filling up the registration and agreeing to privacy policy.
	...
	\item [G.6] \textbf{Monitor user's health parameters.}
		...
		\item [R.23] The application has to retrieve users' health status every 2 seconds in order to guarantee reaction time of 5 seconds.
		\item [R.24] The application is able to send to Data4Help service all the idata already retrieved in live acquisition.
		\item [R.25] The application gets from Data4Help service all the hystorical data stored about the user.
		\end{enumerate}
	...
	\item [G.7] \textbf{Send an ambulance to user's location whenever certain parameters are below the threshold.}
		...
		\item [R.27] The application has made a ambulance request to the nearest hospital, if parameters are critical.
...
\item[•]{\Large Track4Run}
	...
	\item [G.5] \textbf{Retrieve user's position and health status.}
		\begin{enumerate}
		\item [R.30] Allow users to be tracked from Track4Run filling up the registration and agreeing to privacy policy.
		\item [D.4] Devices used to monitor individuals always report correct values.
		\item [R.21] The application has to interact with Smartwatch/Smartphone APIs in order to retrieve location and health status.
		\end{enumerate}
	...
	\item [G.8] \textbf{Allow user to manage a run.}
		...
		\item [G.8.1] \textbf{Allow promoters to define a path for the run.}
			\begin{enumerate}
			\item [R.33] Allow promoters to define a path for the run by selecting the routes inside a map.
			\item [D.14] The path defined by the organizer actually exist.
			\end{enumerate}
			
		\item [G.8.2] \textbf{Allow promoters to invite athletes to the run.}
			\begin{enumerate}
			\item [R.34] Allow promoters to invite athlete to partecipate to a run.
			\end{enumerate}
	\end{enumerate}
	
	\item [G.9] \textbf{Allow athlete to enroll on a specific run.}
		\begin{enumerate}
		\item [R.35] Allow user to see all the runs where the user has been invited to take part.
		\item [R.36] Allow user to make a partecipating request for a run where the user is not already invited.
		\item [D.16] If an user enroll to a run then he also participates to the run.
		\end{enumerate}
	...
	\item [G.10] \textbf{Allow spectators to watch in real time the position of every athletes in a specific run.}
		...
		\item [R.37] Allow user to select a race to be viewed.
		\item [R.38] The application must be able to request to Data4Help the positions of all the other athletes involved.
		\item [R.39] The application must be able to receive and display the positions of all the other athletes involved.
...
Event Flow       & \begin{enumerate}
			...
			\item The system checks if the user accepted the individual treatment of data policy.                
             \item The system accepts the log in request.
        \end{enumerate}\\ \hline
...
\item If the user inserts invalid log in credentials a warning is generated saying the credentials are invalid.
 \item If the user logs in with an already existing account for which he didn't accept the individual treatment of data policy then a warning is displayed stating that the policy must be accepted in order to use the application. For this reason the user is asked again to accept the policy. 
\end{itemize}\\ \hline
\end{tabular}
\end{table}
\FloatBarrier
...
Anyhow users can read and agree regulatory policy first.
\subsubsection{Maintainability}
In order to guarantee maintainability the entire software project is based on Data4Help primitives (as data request, exchange and classification) that must be developed with accuracy and must be certificated. By using or extending fundamental primitives it is possible to construct incremental and interchangeable blocks that can be used to perform all the other services requested.
...
AutomatedSOS and Track4Run applications are developed as multi platform technology so either iOS or Android devices can run these two apps. (remember that AutomatedSOS requires Smartwatch device).
