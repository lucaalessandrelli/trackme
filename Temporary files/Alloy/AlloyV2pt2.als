...
fact NoSharedHealthStatusBetweenAcquisitionSetData {
	 all s1,s2: AcquisitionSetData | all h : HealthStatus | (h in s1.healthStatusAcquisition and h in s2.healthStatusAcquisition) implies s1 = s2
}
...
fact MapLocationsAreRunnersLocations {
	all l: Location | all r: Run | l in r.map.athletesLocation  implies l in r.athletes.retrievedData.locationAcquisition
}
...
run show for 6  but exactly 3 Run
