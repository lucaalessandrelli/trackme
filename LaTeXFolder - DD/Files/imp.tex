...
\begin{itemize}
\item Inefficient algorithms: this test is addressed in the largest path to RequestManager component and how it manages the requests in order to speed up the answers.
...
\item Hardware issues: this test is addressed to the hardware of devices that our system can not control, such as GPS system and HS system, in order to check if these data are subjected to a relevant noise.
\item Network issues: test WebServer component in order to check how much load connection requires to supply on requests and understand if is necessary to expand the broadband.
\end{itemize}
